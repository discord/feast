...
class IcebergTableCreateDisposition(ConstrainedStr):
...
class IcebergOfflineStoreConfig(FeastConfigBaseModel):
    """Offline store config for GCP Iceberg"""

    type: Literal['iceberg'] = 'iceberg'
    """ Offline store type selector"""

    dataset: StrictStr = 'feast'
    """ (optional) Iceberg Dataset name for temporary tables """

    project_id: Optional[StrictStr] = None
    """ (optional) GCP project name used for the Iceberg offline store """
    billing_project_id: Optional[StrictStr] = None
    """ (optional) GCP project name used to run the bigquery jobs at """
    location: Optional[StrictStr] = 'US'
    """ (optional) GCP location name used for the Iceberg offline store.
    The location defaults to the ``US`` multi-regional location, and the Iceberg tables need to be multi-region.
    """

    gcs_staging_location: Optional[str] = None
    """ (optional) GCS location used for offloading Iceberg results as parquet files."""

    table_create_disposition: Optional[IcebergTableCreateDisposition] = None
...
class IcebergOfflineStore(OfflineStore):
    @staticmethod
    @log_exceptions_and_usage(offline_store='iceberg')
    ...
    def get_historical_features(
        ...
        def query_generator() -> Iterator[str]:
            ...
            # Build a query context containing all information required to template the Iceberg SQL query
            ...
            # Generate the Iceberg SQL query from the query context
...
class IcebergRetrievalJob(RetrievalJob):
    def __init__(
        ...
        if self.config.offline_store.gcs_staging_location:
            self._gcs_path = (
                self.config.offline_store.gcs_staging_location + f'/{self.config.project}/export/' + str(uuid.uuid4())
            )
        else:
            self._gcs_path = None
    ...
    def _to_df_internal(self, timeout: Optional[int] = None) -> pd.DataFrame:
        with self._query_generator() as query:
            df = self._execute_query(query=query, timeout=timeout).to_dataframe(create_bqstorage_client=True)
            return df
    ...
    def to_bigquery(
        ...
        """
        Synchronously executes the underlying query and exports the result to a Iceberg table. The
        underlying Iceberg job runs for a limited amount of time (the default is 30 minutes).

        Args:
            job_config (optional): A bigquery.QueryJobConfig to specify options like the destination table, dry run, etc.
            timeout (optional): The time limit of the Iceberg job in seconds. Defaults to 30 minutes.
        ...
        if not job_config.dry_run and self.on_demand_feature_views:
            job = self.client.load_table_from_dataframe(self.to_df(), job_config.destination)
            job.result()
    ...
    def _execute_query(
        ...
        if job_config and job_config.dry_run:
            print('This query will process {} bytes.'.format(bq_job.total_bytes_processed))
            return None
...
def _get_entity_schema(client: Client, entity_df: Union[pd.DataFrame, str]) -> Dict[str, np.dtype]:
    if isinstance(entity_df, str):
        entity_df_sample = client.query(f'SELECT * FROM ({entity_df}) LIMIT 0').result().to_dataframe()
...
def _get_entity_df_event_timestamp_range(
    ...
    if type(entity_df) is str:
        ...
        if entity_df_event_timestamp_range[0] is None or entity_df_event_timestamp_range[1] is None:
...
def _get_bigquery_client(project: Optional[str] = None, location: Optional[str] = None) -> bigquery.Client:
    try:
        client = bigquery.Client(project=project, location=location, client_info=get_http_client_info())
...
def arrow_schema_to_bq_schema(arrow_schema: pyarrow.Schema) -> List[SchemaField]:
    ...
    for field in arrow_schema:
        ...
        bq_schema.append(SchemaField(name=field.name, field_type=detected_type, mode=detected_mode))
...
CREATE TEMP TABLE {{ featureview.name }}__cleaned AS (
    ...
    */
    ...
    {{ featureview.name }}__subquery AS (
        ...
        WHERE {{ featureview.timestamp_field }} <= CAST(UNIX_MILLIS('{{ featureview.max_event_timestamp }}') AS INT64)  -- Use UNIX_MILLIS
        {% if featureview.ttl == 0 %}{% else %}
        AND {{ featureview.timestamp_field }} >= CAST(UNIX_MILLIS('{{ featureview.min_event_timestamp }}') AS INT64)  -- Use UNIX_MILLIS
        {% endif %}
    ),
    ...
    {{ featureview.name }}__base AS (
        ...
        ON TRUE
            AND subquery.event_timestamp <= CAST(UNIX_MILLIS(entity_dataframe.entity_timestamp) AS INT64)

            {% if featureview.ttl == 0 %}{% else %}
            AND subquery.event_timestamp >= CAST(UNIX_MILLIS(Timestamp_sub(entity_dataframe.entity_timestamp, interval {{ featureview.ttl }} second)) AS INT64)
            {% endif %}
